...
- **Persistent OAuth Session**: Silent background token refresh flow using PKCE in the main process to maintain the Spotify session indefinitely without user intervention.
- **Main Process Caching**: Network requests and file-based caching for LRCLIB are handled in the Node.js main process to bypass browser CORS restrictions and dramatically improve performance.
